...
std::string NetworkRequestChannel::pipe_name(Mode _mode) {
...
void NetworkRequestChannel::open_write_pipe(const char * _pipe_name) {
...
void NetworkRequestChannel::open_read_pipe(const char * _pipe_name) {
...
NetworkRequestChannel::NetworkRequestChannel(const std::string _name, const Side _side) :
my_name(_name), my_side(_side), side_name((_side == NetworkRequestChannel::SERVER_SIDE) ? "SERVER" : "CLIENT")
{
...
NetworkRequestChannel::~NetworkRequestChannel() {
    ...
    close(wfd);
    close(rfd);
    if (my_side == NetworkRequestChannel::SERVER_SIDE) {
