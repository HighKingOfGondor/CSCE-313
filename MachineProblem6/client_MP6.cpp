...
void* request_thread_function(void* arg) {
	 ...
	 */
    RT_Params rt = *(RT_Params *) arg;
    SafeBuffer *buffer = rt.buffer;
    int number_of_requests = rt.number_of_requests;
    string name = rt.name;
...
void* worker_thread_function(void* arg) {
     ...
     */
    WT_Params wt = *(WT_Params *)arg;
    SafeBuffer *buffer = wt.buffer;
    RequestChannel *worker = wt.requests;
    while(true) {
        string name = buffer->retrieve_front();
        if (name == "quit") {
            worker->send_request("quit");
            delete worker;
            break;
        }
    }
